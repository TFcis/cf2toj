...
async def main():
    ...
    args_parser.add_argument(
        ...
        "--enable-dependency",
        action="store_const",
        dest="enable_dependency",
        const=True,
        help="是否啟用polygon子任務依賴",
    )
    ...
    conf["limit"]["default"]["timelimit"] = int(
        root.find("./judging/testset/time-limit").text
    )
    conf["limit"]["default"]["memlimit"] = int(
        int(root.find("./judging/testset/memory-limit").text) / 1024
    )
    ...
    for idx, test in enumerate(root.findall("./judging/testset/tests/"), start=1):
        ...
        if "group" not in test.attrib and groups_enabled:
            logging.warning(
                f"Testdata {test} does not have a group attrib. It might be missing group"
            )
            tasks_group[0] = {
                "weight": 0,
                "data": [],
    ...
    with open(
        os.path.join(tmp_outputpath, "conf.json"), "w", encoding="utf-8"
    ) as conffile:
    ...
    returncode = await run_and_wait_process(
        "tar",
        *[
            "-C",
            tmp_outputpath,
            "-cJf",
            os.path.join(
                os.path.dirname(tmp_outputpath),
                "{}.tar.xz".format(os.path.basename(tmp_outputpath)),
            ),
            "http",
            "res",
            "conf.json",
